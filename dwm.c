...
enum { SchemeNorm, SchemeSel, SchemeHid,
       SchemeLayout, TagSchemeNorm }; /* color schemes */
...
drawstatus(Monitor* m)
{
    ...
    for (i = last; i >= 0; i--) {
        if (stext[i] == '^') {
            ...
            drw_text(drw, m->ww-tw-2*sp, 0, TEXTW(substr)+status_space, bh, (lrpad+status_space)/2, substr, 0);
            drw_rect(drw, m->ww-tw+status_space/2-2*sp, bh-3, TEXTW(substr), 3, 1, 0);
            index++;
        }
    }
    if (last >= 0) {
...
loadxrdb()
{
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        XRDB_LOAD_COLOR("dwm.color0", col_black);
        XRDB_LOAD_COLOR("dwm.color7", col_gray);
        XRDB_LOAD_COLOR("dwm.color15", col_white);
        XRDB_LOAD_COLOR("dwm.color1", col_red);
        XRDB_LOAD_COLOR("dwm.color3", col_orange);
        XRDB_LOAD_COLOR("dwm.color11", col_yellow);
        ...
        XRDB_LOAD_COLOR("dwm.activeColor", col_sel);
      }
    }
  }
